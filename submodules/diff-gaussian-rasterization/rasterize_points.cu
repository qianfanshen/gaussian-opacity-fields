	...
	const torch::Tensor& feats3D)
{
  ...
  torch::Tensor out_feats = torch::full({NUM_FEATS_CHANNELS, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		feats3D.contiguous().data_ptr<float>(),
		...
		out_feats.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_feats, radii, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dfeats3D = torch::zeros({P, 2}, means3D.options());
  ...
  {  
	  ...
	  dL_dfeats3D.contiguous().data<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dview2gaussian, dL_dfeats3D);
}
	...
	const torch::Tensor& feats3D)
{
  ...
  return std::make_tuple(rendered, out_color, out_feats, out_alpha_integrated, out_color_integrated, radii, geomBuffer, binningBuffer, imgBuffer);
}